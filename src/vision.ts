 ...
 *
 * Every constant and rule here is transcribed from docs/claude-vision-spec.md
 * (itself a verbatim extract of the official docs). Section references in the
 * comments point at the paragraph a number came from. If this file and that
 ...
 * screenshots (§4): 1920×1080 lands on 1456×819, not 1568×882.
 */
...
export interface ViewOptions {
   ...
   * below this width". It is compared against `scale * tier.maxEdge`, so for
   * the 1568px-wide screenshots this tool exists for it is simply the fitted
   * width; for other sizes it is the same shrink expressed on a common ruler.
   *
   * 900 out of 1568 is ~0.57 scale. Below that, 14px body text lands under
   * 8px and its baseline stops being recoverable, which is the whole reason to
   * measure. A floor on scale rather than on absolute pixels is what separates
   * 1568×7698 (fits at 20% — unusable) from 1080×1920 (fits at 76% — fine).
   */
  minLongEdge?: number;
  /**
   * Vertical overlap between slices. A feature landing exactly on a seam is
   * unmeasurable in either neighbour, and 40px covers a line of body text plus
   * its leading.
   */
  overlap?: number;
}
...
function tile(width: number, height: number, sliceHeight: number, overlap: number): Box[] {
  ...
  for (let y = 0; y < height; y += step) {
    ...
    if (y + sliceHeight >= height) {
      boxes.push({ x: 0, y: Math.max(0, height - sliceHeight), width, height: Math.min(sliceHeight, height) });
      break;
    }
 ...
 * a picture that looks fine in a viewer and is worthless to measure from. Six
 * readable crops beat one 319px-wide smear of a 7698px page, and §10 of the
 * spec says the same: "The correct response to a tall image is to crop it into
 * slices, not to scale it."
 *
 ...
 * downscaled however badly that reads.
 */
